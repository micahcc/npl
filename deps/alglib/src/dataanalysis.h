...
/*************************************************************************
This  structure   is  used  to  store  results of the expectaction 
...
* TerminationType contains completion code, negative on failure, positive
  on success
* K contains number of clusters found
...
FORMAL DESCRIPTION OF FIELDS:
    NPoints         number of points, >=0
    NFeatures       number of variables (dimensions), >=1
    TerminationType completion code:
                    * -5 if  distance  type  is  anything  different  from
                         Euclidean metric
                    * -3 for degenerate dataset: a) less  than  K  distinct
	...
	cov				covariance matrices each is NFeatures by NFeatures, this
					array stores K covariance matrices with the first 
					stored in [0..NFeatures-1,0..NFeatures-1], the second in 
					[NFeatures..2*NFeatures,0..NFeatures-1]. 
					array[0..NFeatures*K-1,0..NFeatures]
	mu				matrix storing mean (middle of each region), each row 
					forms a NFeatures-dimensional mean vector. 
					array[0..K-1,0..NFeatures-1]
	prob			matrix with the probability of each point being in a each 
					of K groups. 1 Row for each point. array[0..NPoints-1,0..NFeatures-1]
	cidx			ownership/classification of each point. array[0..NPoints-1]
					
  -- ALGLIB --
     Copyright 27.11.2012 by Bochkanov Sergey
*************************************************************************/
class _expmaxreport_owner
{
public:
    _expmaxreport_owner();
    _expmaxreport_owner(const _expmaxreport_owner &rhs);
    _expmaxreport_owner& operator=(const _expmaxreport_owner &rhs);
    virtual ~_expmaxreport_owner();
    alglib_impl::expmaxreport* c_ptr();
    alglib_impl::expmaxreport* c_ptr() const;
protected:
    alglib_impl::expmaxreport *p_struct;
};
class expmaxreport : public _expmaxreport_owner
{
public:
    expmaxreport();
    expmaxreport(const expmaxreport &rhs);
    expmaxreport& operator=(const expmaxreport &rhs);
    virtual ~expmaxreport();
    ae_int_t &npoints;
    ae_int_t &nfeatures;
    ae_int_t &terminationtype;
    ae_int_t &k;
...
/*************************************************************************
This  function  sets expectation maximization properties : number of 
restarts and maximum number of iterations per one run.
...
INPUT PARAMETERS:
  ...
  -- ALGLIB --
     Copyright 07.05.2013 by Micah Chambers
*************************************************************************/
void clusterizersetexpmaxlimits(const clusterizerstate &s, const ae_int_t restarts, const ae_int_t maxits);
...
/*************************************************************************
This function performs clustering by expectation maximization algorithm.

INPUT PARAMETERS:
    S       -   clusterizer state, initialized by ClusterizerCreate()
    K       -   number of clusters, K>=0.
                K  can  be  zero if you  want  the  algorithm to  try  and 
                guess the number of clusters. 
                
OUTPUT PARAMETERS:
    Rep     -   clustering results; see description of ExpMaxReport
                structure for more information.

  -- ALGLIB --
     Copyright 27.04.2013 by Bochkanov Sergey
*************************************************************************/
void clusterizerrunexpmax(const clusterizerstate &s, const ae_int_t mink, const ae_int_t maxk, const ae_bool repinit, expmaxreport &rep);
...
void clusterizersetkmeanslimits(clusterizerstate* s,
     ...
     ae_int_t maxits,
     ae_state *_state);
void clusterizersetexpmaxlimits(clusterizerstate* s,
     ae_int_t restarts,
     ae_int_t maxits,
     ae_state *_state);
...
void clusterizerrunkmeans(clusterizerstate* s,
     ...
     kmeansreport* rep,
     ae_state *_state);
void clusterizerrunexpmax(clusterizerstate* s,
     ae_int_t maxk,
     ae_int_t mink,
	 ae_bool repinit,
     expmaxreport* rep,
...
void kmeansgenerateinternal(/* Real    */ ae_matrix* xy,
     ...
     ae_state *_state);
void expmaxgenerateinternal(/* Real    */ ae_matrix* xy,
     ae_int_t npoints,
     ae_int_t ndims,
     ae_int_t mink,
     ae_int_t maxk,
...
ae_bool _expmaxreport_init(void* _p, ae_state *_state, ae_bool make_automatic);
ae_bool _expmaxreport_init_copy(void* _dst, void* _src, ae_state *_state, ae_bool make_automatic);
void _expmaxreport_clear(void* _p);
void _expmaxreport_destroy(void* _p);
