...
int main()
{
	cerr << "This tests that everything still behaves when multiple identical "
	...
	// insert again
	cerr << "Inserting all the points again" << endl;
	for(size_t ii=0; ii<NUMSAMPLE; ii++) {
		tree.insert(points[ii], datas[ii]);
	}
	// insert some more again, in reverse order
	cerr << "Inserting some more points" << endl;
	for(size_t ii=NUMSAMPLE-1; ii>5; ii--) {
		tree.insert(points[ii], datas[ii]);
	}
	std::cerr << "Done!" << std::endl;
